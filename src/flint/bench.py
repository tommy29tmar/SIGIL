...
def render_report(manifest_path: Path, out_path: Path | None = None) -> str:
    ...
    title = str(manifest.get("title") or "Flint Benchmark Matrix")
    ...
    lines = [
        f"# {title}",
        "",
        f"_Generated by `flint bench report` on {generated_at}._",
        "",
    ]
...
def render_portability_report(tasks_path: Path, run_paths: list[Path], out_path: Path | None = None) -> str:
    ...
    lines = [
        ...
        "",
        f"_Generated by `flint bench portability-report` on {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}._",
        "",
