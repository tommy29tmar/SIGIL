...
PROSE_CAVEMAN_DIRECTIVE = (
    ...
    "switch to micro-prose. Two flavors: "
    "(a) pure vague prompts (e.g. 'site is slow', 'login broken', 'help') "
    "— ask only the missing input needed for next action; no speculation. "
    "(b) concrete error prompts with an identifiable pattern (TypeError, "
    "NullPointerException, CORS, stack trace, named failure mode) — name "
    "1 likely cause + 1 safe defensive check or quick fix, then ask for "
    "the exact call site or log. Budget either flavor: max 2-3 lines. "
